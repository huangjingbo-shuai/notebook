...
2. 替换，`%s/内容/后内容/`'
3. 。一般用的比较少。
4. 查找过后会高亮，通过`：noh`回车取消高亮。
5. 取消行号：`set nonu`
6. 有的时候修改文档的权限会不够，故通常在命令后面加一个`！`。
# VIM的三种模式的切换
...
1. 移除软件（不保留配置）命令:`dpkg -P 包名`
实例:dpkg -P zabbix-release
1. 查找包的详细信息。命令:`dpkg -s 包名`
实例:dpkg -s zabbix-release
1. 列出deb包的内容。命令:`dpkg -c 包名.deb 列出 deb 包的内`
...
1. 如果需要调用图形界面程序可以使用 -X 选项
eg:`ssh -X ldz@192.168.0.1`
1. 如果客户机的用户名和服务器的用户名相同，登录时可以省略用户名。eg:`ssh 192.168.0.1`
2. 还要说明的是，SSH服务的默认端口是22，也就是说，如果你不设置端口的话登录请求会自动送到远程主机的22端口。我们可以使用 -p 选项来修改端口号，比如连接到服务器的1234端口：`ssh -p 1234 ldz@192.168.0.1`。
3. 客户机必须要知道服务器的ip地址。可以在服务器端电脑上利用 `ifconfig`命令查看该机的ip地址。
4. 如果是第一次登录远程主机，系统会给出下面提示：
   ![alt text](./.assets_IMG/Linux/image-55.png)
5. 意思是，该远程主机的真实性无法确定，其公钥指纹为 SHA256:FFobshqrGOachj7Xp4LsJ9+xkNBlyyOe8ZIPl7K+qQI，确定想要继续连接吗？
输入yes即可。这时系统会提示远程主机被添加到已知主机列表。
1. 然后会要求我们输入远程主机的密码，输入的密码正确就可以成功登录了。命令提示符会修改为远程主机的提示符，现在开始，终端中输入的命令都将在服务器中执行。
## 二、公钥登陆
...
## 针对ubuntu系统窗口总是隐藏到上面
1. `Alt+F7`拖动
## 关于ubuntu系统的校园网问题及其解决办法
1. 今天是2025年十月十七日，我无法使用校园网已经好几天了，很不方便。但是我在CSDN上找了但是都没有解决。
2. 问题：连接上了校园网，但是无法触发连接界面以及无法上网的问题，wifi图标显示的是`?`。
3. 解决办法：重启一下网络服务。`sudo nmcli networking off`加`sudo nmcli networking on`，问题解决
## 水下相机的使用
1. 用水下相机，这个是带橡胶管的那个。
2. 使用的办法是`roslaunch realsense2_camera rs_camera.launch`